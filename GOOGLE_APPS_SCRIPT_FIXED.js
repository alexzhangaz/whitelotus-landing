/**
 * Google Apps Script for White Lotus Software Trial Requests - FIXED VERSION
 * 
 * 设置步骤：
 ...
 * 4. 删除默认代码，粘贴此代码
 * 5. 保存并部署为 Web 应用
 * 6. 将部署后的 URL 替换到 BookingPage.tsx 中
 */
...
function doPost(e) {
  try {
    ...
    try {
      ...
      console.log('JSON parse error, trying form data:', parseError);
      ...
      try {
        ...
        if (formData && formData.name) {
          data = {
            ...
            source: formData.source || 'booking_page'
          };
        } else {
    ...
    sheet.appendRow(row);
    console.log('Data saved to sheet:', row);
...
function sendEmailNotification(data) {
  try {
    ...
    console.log('Email notification sent successfully to:', CONFIG.ADMIN_EMAIL);
  } catch (error) {
